# Multi Browser 🚀

Aplikasi browser multi-tab yang ringan dan cepat untuk berbagai kebutuhan browsing sehari-hari.

![Multi Browser Preview](https://via.placeholder.com/800x400/3a86ff/ffffff?text=Multi+Browser+Screenshot)

## ✨ Fitur Utama

- **Multi-tab browsing** - Buka banyak situs sekaligus dalam satu window
- **Mode incognito** - Browsing privat tanpa menyimpan history
- **Blokir iklan** - Built-in ad blocker untuk pengalaman browsing bersih
- **Manajer password** - Simpan dan kelola password dengan aman
- **Sync antar perangkat** - Sinkronisasi bookmark dan history
- **Ekstensi browser** - Dukungan untuk ekstensi populer
- **Ringan & cepat** - Optimalkan penggunaan RAM dan CPU

## 📥 Download Aplikasi

<summary><b>📱 Versi Android</b></summary>

### Android Requirements
- Android 7.0 atau lebih tinggi
- RAM minimal 2GB

### Download Links Android
- [Download APK v1.2.0](https://drive.google.com/uc?export=download&id=1j1Ymb8ATT8VQHLNAyL5Upfsb1wc-8PUe)

### Cara Install Android
1. Download file APK dari link di atas
2. Buka file APK yang sudah didownload
3. Izinkan instalasi dari sumber tidak dikenal (jika diminta)
4. Ikuti petunjuk instalasi
5. Buka aplikasi dan mulai browsing!
